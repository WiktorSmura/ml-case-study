...
def parse_birth(bn):
    ...
    dates = pd.to_datetime(pd.DataFrame({"year": 1900 + yy, "month": mm, "day": dd}), errors="coerce")
    return dates, gender
...
colors_bar = [C["blue"], C["indigo"], C["purple"], C["teal"], C["green"], C["amber"], C["red"], C["slate"]]
...
style_ax(ax, title="Czech Financial Dataset — Table Sizes", xlabel="Number of rows", ylabel="")
ax.invert_yaxis()
...
for title, x, y, cols in nodes:
    rect = mpatches.Rectangle(
        ...
        box_w,
        box_h,
        edgecolor="black" if not DARK else C["text"],
        facecolor="white" if not DARK else C["surface"],
        lw=1.5,
        zorder=3,
    )
    ax.add_patch(rect)
    ax.text(
        ...
        fontsize=10,
        fontweight="bold",
        fontstyle="italic" if "Demograph" not in title and "Permanent" not in title else "normal",
        color="black" if not DARK else C["text"],
        zorder=4,
    )
    ax.text(
        x - box_w / 2 + 0.1,
...
for src, dst in edges:
    ...
    if src in ["Loan", "Permanent\norder", "Transactions"] and dst == "Account":
        ...
        ax.plot(
            [x0 + box_w / 2, 2.75, 2.75, x1 - box_w / 2],
            [y0, y0, y1, y1],
            color="black" if not DARK else C["text"],
            lw=1.5,
            zorder=2,
        ...
        ax.plot(
            [x0 + box_w / 2, 5.25, 5.25, x1 - box_w / 2],
            [y0 - 0.2, y0 - 0.2, y1, y1],
            color="black" if not DARK else C["text"],
            lw=1.5,
            zorder=2,
        ...
        ax.plot(
            [x0 - box_w / 2 + 0.2, 7.5, 7.5, x1 + box_w / 2],
            [y0 - box_h / 2, 3.0, y1, y1],
            color="black" if not DARK else C["text"],
            lw=1.5,
            zorder=2,
        )
...
ax.set_title(
    "Entity-Relationship Overview — Czech Financial Dataset",
    fontsize=13,
    fontweight="bold",
    color=C["text"] if DARK else "black",
    pad=12,
...
if DARK:
    ax.set_facecolor(C["surface"])
bars = ax.bar(
    [status_labels[s] for s in ["A", "B", "C", "D"]], status_counts.values, color=status_colors, edgecolor="none", width=0.6
)
...
ax2.set_title(
    "Finished Loans — Binary Target\n(Class Balance)", fontsize=12, fontweight="bold", color=C["text"] if DARK else "black"
)
center_text = f"n = {len(finished)}"
ax2.text(0, 0, center_text, ha="center", va="center", fontsize=11, color=C["text"] if DARK else "black", fontweight="bold")
...
ax.set_xticks(x)
ax.set_xticklabels([f"{int(d)}m" for d in dur_vals], fontsize=8, color=C["text"] if DARK else "black")
...
ax.set_xticks(x)
ax.set_xticklabels(["Good (A)", "Bad (B)"], fontsize=10, color=C["text"] if DARK else "black")
for i, (mv, fv) in enumerate(zip(m_vals, f_vals)):
    total = mv + fv
    ax.text(i, mv / 2, f"M {mv}", ha="center", va="center", fontsize=9, color=C["text"], fontweight="bold")
    ax.text(i, mv + fv / 2, f"F {fv}", ha="center", va="center", fontsize=9, color=C["text"], fontweight="bold")
...
for at in autotexts:
    ...
    at.set_fontweight("bold")
    at.set_color(C["text"] if DARK else "black")
ax.set_title("Transaction Type Mix", fontsize=12, fontweight="bold", color=C["text"] if DARK else "black")
ax.text(0, 0, f"{len(trans):,}\ntransactions", ha="center", va="center", fontsize=9, color=C["text"] if DARK else "black")
...
ax.set_xticks(x)
ax.set_xticklabels(amt_by_type["type_clean"], fontsize=8, color=C["text"] if DARK else "black")
ax.yaxis.set_major_formatter(matplotlib.ticker.FuncFormatter(lambda v, _: f"{v / 1000:.0f}k"))
style_ax(ax, title="Transaction Amount by Type (CZK)", xlabel="", ylabel="Amount (CZK)", legend=True)
ax.tick_params(axis="y", colors=C["text"] if DARK else "black")
...
for bar, v in zip(bars, reg_sal.values):
    ax.text(
        v + 50, bar.get_y() + bar.get_height() / 2, f"{v:,.0f}", va="center", fontsize=8, color=C["text"] if DARK else "black"
    )
...
for i, reg in enumerate(regions):
    ...
    ax.scatter(
        sub["average_salary"], sub["crime_rate96"], color=pal[i % len(pal)], label=reg, s=50, alpha=0.85, edgecolors="none"
    )
style_ax(ax, title="Crime Rate vs Avg Salary", xlabel="Avg Salary (CZK)", ylabel="Crimes / 1000 inhabitants", legend=True)
ax.tick_params(colors=C["text"] if DARK else "black")
if ax.get_legend():
...
for x, lbl, col, sublbl in events:
    ...
    box = mpatches.FancyBboxPatch(
        ...
        linewidth=2.5,
        edgecolor=col,
        facecolor="white" if not DARK else "#F0F4FF",
        zorder=4,
    )
    ax.add_patch(box)

    # Text inside Event Box
    ax.text(x, 3.4, lbl, ha="center", va="center", fontsize=13, fontweight="bold", color=col, zorder=5)
    ...
    ax.text(
        ...
        va="center",
        fontsize=11,
        color="black" if not DARK else "#666",
        zorder=5,
        linespacing=1.4,
        fontweight="bold",
    )
...
ax.text(
    ...
    va="center",
    fontsize=12,
    color="black" if not DARK else "#444",
...
ax.set_title("Temporal Data Leakage", fontsize=20, fontweight="bold", color="black" if not DARK else C["text"], y=0.98)
plt.tight_layout()
...
acc_dist = account[["account_id", "district_id"]].merge(
    district[["district_id", "average_salary", "unemp96", "crime_rate96"]], on="district_id", how="left"
)
...
ax.set_title(
    "Feature Correlations across merged tables",
    pad=20,
    fontsize=14,
    fontweight="bold",
    color="black" if not DARK else C["text"],
...
rows = [
    ...
    (
        "→ Finished A (Good)",
        f"{(loan['status'] == 'A').sum():,}",
        f"  {(loan['status'] == 'A').mean() * 100:.1f}% of all loans",
    ),
    (
        "→ Finished B (Bad)",
        f"{(loan['status'] == 'B').sum():,}",
        f"  {(loan['status'] == 'B').mean() * 100:.1f}% of all loans",
    ),
    ...
    (
        "Gender split (owners)",
        f"M {(owners['gender'] == 'M').mean() * 100:.0f}% / F {(owners['gender'] == 'F').mean() * 100:.0f}%",
        "finished loans subset",
    ),
